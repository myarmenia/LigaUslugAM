<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Password Reset Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are the default lines which match reasons
    | that are given by the password broker for a password update attempt
    | has failed, such as for an invalid token or invalid new password.
    |
    */
    'firm_name'=>'Գործ Կա',
    // ======================newjob.blade====================================================
        'new_projects_on_liga_uslug'=>'Նոր նախագծեր Лига Услуг',
        'the_price_is_negotiated'=>'Գինը սակարկելի',

        'You_are_the_first_to_see_these_orders_respond_quickly' => 'Դուք առաջինն եք, ով տեսնում է այս առաջադրանքները, արձագանքեք!',
        'interesting_hurry_up_with_your_response_now_the_order_is_available_to_a_limited_number_of_performers_but_in_an_hour_we_will_show_the_rest'=>'Հետաքրքի՞ր է: Շտապե՛ք արձագանքել, այժմ պատվերը հասանելի է սահմանափակ թվով կատարողների, բայց մեկ ժամից մենք ցույց կտանք մնացածին:',
        'respond'=>'Արձագանքել',
    // ===================notifyemployerfordeletingtask=====================================================
    'deleting_tasks_due_to_lack_of_feedback'=>'Հարգելի Պատվիրատու, առաջադրանքը ջնջվել է Կատարողների արձագանքի բացակայության պատճառով',
    'dear' =>'Հարգելի',
    'your_task_will_be_deleted_because_no_one_responded_to_the_order'=>'ձեր առաջադրանքը կջնջվի, քանի որ ոչ ոք չի դիմել',
    // ===================rejecttaskexecutor======================================================
    'the_customer_chose_the_right_specialist'=>'Պատվիրատուն ընտրել է համապատասխան  մասնագետին',
    'dear_executor'=>'Հարգելի Կատարող',
    'thank_you_for_your_response_to_your_order'=>'Շնորհակալություն պատվերին արձագանքելու համար',
    'thank_you'=>'Շնորհակալություն',
    'thank_you_for_your_response_to'=>'Շնորհակալություն, որ դիմել եք թիվ',
    'this_time_the_client_chose_the_right_specialist'=>'այս անգամ հաճախորդը ընտրել է համապատասխան մասնագետին։',
    'sincerely'=>'Հարգանքով',

    // =====================clickOnTaskController============================================
    'you_cannot_apply_for_this_job_while_your_status_is_passive'=>'Դուք չեք կարող դիմել այս առաջադրանքի համար, քանի դեռ ձեր կարգավիճակը պասիվ է:',
    'you_have_already_responded_to_this_order'=>'Դուք արդեն արձագանքել եք այս առաջադրանքին',
    'the_cost_of_the_To_service_cannot_be_less_than_from'=>'Մուտքագրեք ծառայության  արժեքի հավաստի դաշտեր',
    //===========================
    'completion_of_work_one_day_left'=>'Առաջադրանքի ավարտին, մնաց մեկ օր',
    'follow_the_link'=>'Անցեք հղումով',
    'thank_you_in_advance_for_submitting_your_work_on_time'=>'Նախապես հայտնում ենք մեր  շնորհակալությունը, առաջադրանքը ժամանակին ներկայացնելու համար։',
    'number'=>'Թիվ',
    // ================
    'the_task_will_be_deleted'=>'Առաջադրանքը կջնջվի երկու օրից',
    //
    'forget_password'=>'Գաղտնաբառի վերականգնում',
    'you_have_successfully_changed_your_password'=>'Ձեր գաղտնաբառը փոխված է',
    // update avatar
    'you_have_successfully_updated_your_profile_avatar'=>'Ձեր պրոֆիլի նկարը թարմացվել է:',


];
