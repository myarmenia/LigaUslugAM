        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-3 col-xs-12  mb-3">
                   <h3>Уважаемый(ая) {{$rating->tasks->users->name.' '.$rating->tasks->users->last_name}}</h3>
                    <p class="m-2">Исполнитель {{$rating->tasks->executor_profiles->users->name.' '.$rating->tasks->executor_profiles->users->last_name}} оценил(а) вас и оставил(а) комментарий по задании {{ $rating->tasks->title }}</p>
