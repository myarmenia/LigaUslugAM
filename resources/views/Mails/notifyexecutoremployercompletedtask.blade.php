        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-3 col-xs-12  mb-3">
                   <h3>Уважаемый(ая) {{$task->executor_profiles->users->name.' '.$task->executor_profiles->users->last_name}}</h3>
                    <p class="m-2">Заказчик {{$task->users->name.' '.$task->users->last_name}} завершил заказ по названию {{$task->title}}</p>
