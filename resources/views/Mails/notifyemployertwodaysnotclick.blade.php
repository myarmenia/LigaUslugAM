            ...
            <div class="row">
                <div class="col-sm-3 col-xs-12  mb-3">
                   {{-- <h3>Уважаемый(ая) {{$task->users->name.' '.$task->users->last_name}}</h3>
                    ...
                    <h3>Уважаемый(ая) {{ $task->users->name .' '.$task->users->last_name}} </h3>
                    <p class="m-2">На ваше задаче '{{$task->title}}' нет откликов, если в течение двух дней не будет откликов задача  будет удалена</p>
                    <h5>С уважением</h5>
                ...
                <div style="margin-top:20px"><button type="button" class="btn" style="background-color: #4B9A2D;border-radius:15px;padding:5px 8px; border:none;outline:none"><a href="http://ligauslug.ru/employer_task/772" style="color:white;text-decoration:none">Перейти по ссылке</a></button></div>
            </div>
        </div>
    </body>
</html>
